...
class TestBindActionCreators(unittest.TestCase):
    ...
    def test_wraps_action_creators_with_dispatch_function(self):
        bound_action_creators = bind_action_creators(
            action_creators, self.store['dispatch'])
    ...
    def test_supports_wrapping_single_function_only(self):
        ...
        bound_action_creator = bind_action_creators(
            action_creator, self.store['dispatch'])
    ...
    def test_throws_for_undefined_action_creator(self):
        ...
        self.assertTrue(re.search(
            r'bind_action_creators expected an object or a function, instead received None', str(e.exception)))

    def test_throws_for_a_primative_action_creator(self):
        with self.assertRaises(Exception) as e:
            bind_action_creators('string', self.store['dispatch'])
        self.assertTrue(re.search(
            'bind_action_creators expected an object or a function, instead received <class \'str\'>', str(e.exception)))
