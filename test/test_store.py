...
class TestStoreClass(unittest.TestCase):
	...
	def test_accepts_and_applies_enhancer(self):
		...
		store = Store(reducers['todos'], preloaded_state=empty_array, enhancer=spy_enhancer)
	...
	def test_throws_if_enhancer_is_neither_undefined_or_a_function(self):
		with self.assertRaises(Exception):
			Store(reducers['todos'], enhancer={})
		with self.assertRaises(Exception):
			Store(reducers['todos'], enhancer=[])
		with self.assertRaises(Exception):
			Store(reducers['todos'], enhancer=False)
		try:
			Store(reducers['todos'], enhancer=None)
			Store(reducers['todos'], enhancer=lambda reducer, **kwargs: Store(reducer, **kwargs))
			Store(reducers['todos'])
			Store(reducers['todos'], preloaded_state=[])
			Store(reducers['todos'], preloaded_state={})
		except Exception:
